 ...
 */
public class SMSDTLSConnector implements Connector {

	private static final Logger LOGGER = Logger.getLogger(SMSDTLSConnector.class.getCanonicalName());
	...
	private final BlockingQueue<RawData> inboundMessages;
	 ...
	 */
	public SMSDTLSConnector(DtlsConnectorConfig configuration, SmsSocket smsSocket, SmsSocketAddress smsSocketAddress) {
		this(configuration, null, smsSocket, smsSocketAddress);
	}
	 ...
	 * @throws NullPointerException if the configuration is <code>null</code>
	 */
	public SMSDTLSConnector(DtlsConnectorConfig configuration, ConnectionStore connectionStore, SmsSocket smsSocket, SmsSocketAddress smsSocketAddress) {
		...
		this.inboundMessages = new LinkedBlockingQueue<>(config.getOutboundMessageBufferSize());
...
this.maximumTransmissionUnit = 140;			
	...
	public void receive(RawData msg) {
		if (msg == null) {
			...
			if (queueFull) {
				LOGGER.log(Level.WARNING, "Inbound message queue is full! Dropping outbound message to peer [{0}]",
	...
	private void receiveNextDatagramFromNetwork() throws IOException {
		...
		try {
			message = inboundMessages.take(); // Blocking
